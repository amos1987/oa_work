  ...
  router.get('/', (req, res)=>{
	    ...
	    db.query(`SELECT * FROM project;\
					...
					if (err) {
					 ...
					 res.render('./director/project.ejs',{project,nav_project:"ok"});		  	  
					}else {
						var i;
						...
						for ( i=0;i<project[0].length;i++){  
							...
							if(project[0][i].state=="ongoing"){
							  ...
							  if(parseInt(i)+1==project[0].length){
								   res.render('./director/project.ejs',{project,results,nav_project:"ok"});
							  };
							}
					}
				});
...
//创建项目
  router.get('/new', (req, res)=>{
	    var username=req.session['sess_username'];
	    db.query(`SELECT * FROM eadmin WHERE username!='${username}'`, (err, eadmin) => {
					if (err) {
						console.error(err);
						res.status(500).send('database error').end();
					} else if(eadmin.length==0){
					 res.render('./director/project_new.ejs',{nav_project:"ok"});		  	  
					}else {
						res.render('./director/project_new.ejs',{eadmin,nav_project:"ok"});   
					}
		});
	    ...
	    db.query(`SELECT * FROM project WHERE p_number='${oP_number}';\
					...
					if (err) {
						...
						resure.push({from:from,to:to,oCycle:oCycle});
						 ...
						 res.render('./director/project_detailed.ejs',{project,resure,nav_project:"ok"});	
	    ...
	    db.query(`SELECT p_number FROM project WHERE p_number='${oP_number}';\
					...
					if (err) {
						...
						res.status(500).send('database error').end();
						 ...
						 res.render('./director/project_d_z.ejs',{project,nav_project:"ok"});	
	  ...
	  //查询参与人员
	    ...
	    db.query(`SELECT p_name,p_number FROM project WHERE p_number='${oP_number}';\
					...
					if (err) {
						...
						res.render('./director/project_d_l.ejs',{project,nav_project:"ok"});   	  
					}else {
										var i;
										var results=new Array();
										for ( i=0;i<project[2].length;i++){ 
											...
											res.render('./director/project_d_l.ejs',{project,nav_project:"ok",results});   
...
//任务详细
	  ...
	  db.query(`SELECT  *  FROM  plan WHERE choose_time<='${timestamp_0}' AND t_number='${t_number}';\
					...
					SELECT username,name_zh  FROM eadmin WHERE username IN (SELECT DISTINCT account FROM task WHERE p_number='${oP_number}');\
					SELECT  name_zh,type  FROM eadmin WHERE  username IN (SELECT  account  FROM  task WHERE t_number='${t_number}' );`, (err, project) => {
		  		if (err) {
					console.error(err);
					res.status(500).send('database error').end();
		  		} else{	
			 ...
			 res.render('./director/project_d_lc.ejs',{project,log_list_all, nav_project:"ok",resure:{id:id,p_name:p_name,t_name:t_name,t_number:t_number,from:from,to:to,cycle:cycle,detailed:detailed,last:last,t_plan:t_plan,unfinished:unfinished,state: state,actual:actual,consuming:consuming,p_number:oP_number}}); 
				}
	  })	
...
//项目总结-get
  router.get('/summary', (req, res)=>{
	   ...
	   db.query(`SELECT * FROM summary WHERE p_number='${oP_number}' AND account='${username}'`, (err, data) => {
					...
					res.render('./director/summary.ejs',{data,nav_project:"ok",p_number:oP_number});   	
					}
