...
//拦截跳转	
 router.use('/',(req, res,next)=>{
	 if(req.session['sess_type']=='director'){
				next();
			}else if(req.session['sess_type']=='controller'){
 ...
 router.get('/', (req, res)=>{
	  ...
	  //查询任务表-查询待审核-有审核申请的  
	  	db.query(`SELECT p_number,p_name FROM project;\
						SELECT  *  FROM task WHERE  state='ing_true' OR state='review' AND account!='${username}' ;\
						SELECT * FROM eadmin;`, (err, task_data) => { 
						  if (err) {
									console.error(err);
										res.status(500).send('database error').end();
									}else if(task_data[1].length==0){
											res.render('./director/d_index.ejs',{nav_index:"ok",task_data});    
									}else{	
										var i;
										...
										for ( i=0;i<task_data[1].length;i++){ 
											...
											res.render('./director/d_index.ejs',{nav_index:"ok",task_data,results});   
	 ...
	 db.query(`SELECT * FROM task WHERE  id=${req.query.id};SELECT p_number,p_name FROM project WHERE  p_number  IN (SELECT p_number  FROM task WHERE   id=${req.query.id});SELECT  p_number,p_name FROM project;`, (err, data) => { 
	 	  if (err) {
			...
			var to=  time_format.format(data[0][0].wheel_e);
			 res.render('./director/i_shen.ejs',{data,from,to,nav_index:"ok"});   
		}
	 });
