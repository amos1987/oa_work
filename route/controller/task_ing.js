 ...
 router.get('/', (req, res)=>{
	  ...
	  //查询任务表-查询待审核-有审核申请的  
	  	db.query(`SELECT p_number,p_name FROM project ;\
						SELECT  *  FROM task WHERE state='ing'  AND account!='${username}' AND p_number IN (SELECT p_number FROM project WHERE head="${username}");\
						SELECT * FROM eadmin;`, (err, task_data) => { 
						  if (err) {
									console.error(err);
										res.status(500).send('database error').end();
									} else if(task_data[1].length==0) {
									  res.render('./controller/task_ing.ejs',{nav_task:"ok",task_data});   
									}else{	
										var i;
										...
										for ( i=0;i<task_data[1].length;i++){ 
											...
											res.render('./controller/task_ing.ejs',{nav_task:"ok",task_data,results});   
  ...
  //[进行中]任务详细--get
	  ...
	  db.query(`SELECT  *  FROM  plan WHERE t_number='${t_number}' AND  choose_time IN (SELECT  choose_time  FROM  plan WHERE choose_time>='${timestamp_24}');\
			 ...
			 res.render('./controller/i_task.ejs',{plan,log_list_all, nav_task:"ok",same_day:same_day,oStar_time:oStar_time}); 
				}
	  })	
