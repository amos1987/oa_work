 ...
 router.get('/create', (req, res)=>{
	  ...
	  db.query(`SELECT * FROM project WHERE (head="${username}" OR FIND_IN_SET('${username}',team)) AND state!="end"`, (err, project) => { 
		  if (err) {
					...
					res.render('./controller/create.ejs',{nav_task_self:"ok",project:project});   	
					}
	  });
...
//创建任务-post数据提交
  router.post('/create', (req, res)=>{
	          ...
	          db.query(`SELECT head FROM project WHERE p_number="${oP_number}" `, (err, data) => { 
				  if (err) {
						console.error(err);
						res.status(500).send('database error').end();
					} else{
						console.log(data);
						if(data[0].head==oAccount){
							oState="ing"
						}else{
							oState="review"
						}
	  ...
	  var username=req.session['sess_username'];
	  	...
	  	//查询进行中的任务
 						db.query(`SELECT  *  FROM task WHERE  account='${username}' AND (state='ing' OR state='review'  OR state='ing_true');\
										SELECT p_number,p_name,head FROM project WHERE p_number IN (SELECT p_number  FROM task WHERE  account='${username}' )`, (err, task_data) => { 
						  if (err) {
									console.error(err);
										res.status(500).send('database error').end();
									}else{	

									res.render('./controller/task_self_list.ejs',{nav_task_self:"ok",task_data,username:username});   
									}
						}); 
  });
...
//查询结束，自己的任务
  router.get('/task_self_end', (req, res)=>{
						  ...
						  if (err) {
									console.error(err);
										res.status(500).send('database error').end();
									  	...
									  	res.render('./controller/task_self_end.ejs',{nav_task_self:"ok",task_data});   
									}else{	
										var i;
										...
										for ( i=0;i<task_data.length;i++){ 
											...
											res.render('./controller/task_self_end.ejs',{nav_task_self:"ok",task_data,results});   
	...
	//查询结束，自己的任务-详细内容
	 ...
	 var p_number=req.query.p_number;//任务单编号	  
	  ...
	  db.query(`SELECT  *  FROM  plan WHERE choose_time<='${timestamp_0}' AND t_number='${t_number}';\
			 ...
			 res.render('./controller/task_self_end_c.ejs',{plan,log_list_all, nav_task_self:"ok"}); 
				}
	  })	
   ...
   //[进行中]任务详细--get
	  ...
	  var id=req.query.id;//任务id  
	 var t_number=req.query.t_number;//任务编号  	  
	  var timestamp_0 =parseInt( Date.parse(new Date(new Date().setHours(0, 0, 0, 0))));//当前日期0点时间戳
	  var timestamp_24 =parseInt( Date.parse(new Date(new Date().setHours(24, 0, 0, 0))));//当前日期24点时间戳
 ...
 //[进行中]主管穿插任务审核
	 ...
	 var t_number=req.query.t_number;//任务编号  	  
	  ...
	  db.query(`SELECT  *  FROM  plan WHERE t_number='${t_number}' AND  choose_time IN (SELECT  choose_time  FROM  plan WHERE choose_time>='${timestamp_24}');\
			 ...
			 res.render('./controller/task_self_task.ejs',{plan,log_list_all,nav_task_self:"ok",same_day:same_day,oStar_time:oStar_time}); 
				}
	  })	
...
//[进行中]任务详细-创建日志-他人
  router.post('/log_other', (req, res)=>{
     ...
     //添加-次日计划
db.query(`INSERT INTO log (t_number,remark,plan,time_old,val_path,choose_time,time,state,tuichi) VALUE  ('${oT_number}','${oRemark}','${oPlan}','${oTime_old}','${oVal_path}','${oChoose_time}','${oStar_time}','${oState}','${oLeave}');\
UPDATE task SET t_plan='${oPlan}',state="ing_true"  WHERE t_number="${oT_number}";\
`, (err, data) => {
										if (err) {
											console.error(err);
											res.status(500).send('database error').end();
